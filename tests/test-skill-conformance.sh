...
check       run-plan "+5 min cron margin"           '\$NOW_MIN \+ 5'
check_fixed run-plan "minute rollover"              'TARGET_MIN" -lt "$NOW_MIN'
check       run-plan "avoid :00/:30"                'TARGET_MIN" -eq 0.*TARGET_MIN" -eq 30|-eq 0 \]\].*-eq 30'
check       run-plan "tz for cron math"             'TZ=America/New_York date'
